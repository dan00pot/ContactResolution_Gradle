...
public class AADS_CRQS016 {
	
	/*########################################################################################################
 	Verify that Enterprise or Equinox  contact matching with call history in case of outcomming calls via device clients
	...
	*Note:Notes:
	...
	final static Logger logger = LogManager.getLogger("AADS_CRQS016");
	...
	@Before
	public void beforetestAADS_CRQS016() throws Exception {
	...
	@Test
	public void AADS_CRQS016() throws Exception {
		logger.info("AADS_CRQS016 - starting...\n");
		try {
			for(int i=0; i<3; i++){
			...
			winClient.oneX_logout(windowsDriverRoot);
			logger.error("AADS_CRQS016 - Failed with Exception:" + exception
					+ "...\n");
			fail("AADS_CRQS016 - Failed - Exception occurs: "
					+ exception.toString());
			assertTrue(false);
		}
		logger.info("AADS_CRQS016 - completed...\n");
	}
