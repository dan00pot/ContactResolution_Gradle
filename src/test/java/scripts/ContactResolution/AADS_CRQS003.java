...
public class AADS_CRQS003 {
	
	/*########################################################################################################
 	Verify that Equinox contact matching with outcomming calls on phone numbers via device clients
 	Verify that Equinox  contact matching with call history in case of outcomming calls via device clients
	...
	*Procedures:
		...
		3. Checking information E, F, G on A, B, C on history
		 
	*Expected result:
		1. information E, F, G show exactly on A, B, C clients while taking outcomming call
		2. information E, F, G show exactly on A, B, C on history
	*Note:Notes:
	...
	final static Logger logger = LogManager.getLogger("AADS_CRQS003");
	...
	@Before
	public void beforetestAADS_CRQS003() throws Exception {
		logger.info("Before test - AADS_CRQS003 - starting...\n");
	...
	@Test
	public void AADS_CRQS003() throws Exception {
		logger.info("AADS_CRQS003 - starting...\n");
		try {
			...
			winClient.oneX_logout(windowsDriverRoot);
			logger.error("AADS_CRQS003 - Failed with Exception_example:" + exception
					+ "...\n");
			fail("AADS_CRQS003 - Failed - Exception occurs: "
					+ exception.toString());
			assertTrue(false);
		}
		logger.info("AADS_CRQS003 - completed...\n");
	}
