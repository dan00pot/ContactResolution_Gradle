...
public class AADS_CRQS004 {
	
	/*########################################################################################################
 	Verify that Equinox  contact matching with mid-calls on phone numbers via device clients
 	Verify that Equinox  contact matching  with call history in case of mid-calls via device clients
	...
	*Procedures:
		...
		2. Checking information E, F, G on A, B, C clients while Lift the headset up
		3. Checking information E, F, G on A, B, C clients in Calls History
		 
	*Expected result:
		1. information E, F, G show exactly on A, B, C clients while Lift the headset up
		2. information E, F, G show exactly on A, B, C clients in Calls History
	...
	*Note:Notes:
	...
	final static Logger logger = LogManager.getLogger("beforetestAADS_CRQS001");
	...
	@Before
	public void beforetestAADS_CRQS004() throws Exception {
		logger.info("Before test - AADS_CRQS004 - starting...\n");
		...
		logger.info("Before test - AADS_CRQS004 - completed...\n");
	}
	...
	@Test
	public void AADS_CRQS004() throws Exception {
		logger.info("AADS_CRQS004 - starting...\n");
		try {
			for(int i=0; i<3; i++){
				
			//range 73950 -> 73952: only have on CM, can using for WMware and AWS
			...
			Thread.sleep(10000);
			winClient.pickUpThePhone(windowsDriverRoot);
			...
			winClient.oneX_logout(windowsDriverRoot);
			logger.error("AADS_CRQS004 - Failed with Exception:" + exception
					+ "...\n");
			fail("AADS_CRQS004 - Failed - Exception occurs: "
					+ exception.toString());
			assertTrue(false);
		}
		logger.info("AADS_CRQS004 - completed...\n");
	}
