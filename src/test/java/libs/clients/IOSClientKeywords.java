...
//				logger.info("Out of office");
	...
	public void verifyContactNameCallHistory(AppiumDriver iOSClientDriver, String CallOrder, String Expected)
			throws Exception {
		try {
			String CalleeName = selenium.getAttribute(iOSClientDriver, iOSClient.CALL_HISTORY_CONTACT_NAME(CallOrder),
					"label");
			logger.info(CalleeName);
			if (CalleeName.equalsIgnoreCase(Expected)) {
				logger.info("PASSED");
			} else {
			...
			logger.error("autoConfigLogin - Failed with Exception: " + e + "...\n");
			throw new Exception("autoConfigLogin - Failed - Exception occurs: " + e);
		}

	}
	
	public void deleteAllHistory(AppiumDriver iOSClientDriver) throws Exception {
		String message = String.format("*** deleteAllHistory - Delete all call history ");
		logger.warn(message);
		try {

			selenium.clickElement(iOSClientDriver, iOSClient.FIRST_SCREEN_EBAR_BTN);
			Thread.sleep(3000);
			selenium.clickElement(iOSClientDriver, iOSClient.FIRST_SCREEN_NAVIGATE_HISTORY_TAB_BTN);
			Thread.sleep(2000);
			selenium.waitUntilElementClickable(iOSClientDriver, iOSClient.HISTORY_SCREEN_OPEN_HISTORY, timeout);
			selenium.clickElement(iOSClientDriver, iOSClient.HISTORY_SCREEN_OPEN_HISTORY);
			selenium.waitUntilElementClickable(iOSClientDriver, iOSClient.HISTORY_SCREEN_DELETE_ALL_HISTORY, timeout);
			selenium.clickElement(iOSClientDriver, iOSClient.HISTORY_SCREEN_DELETE_ALL_HISTORY);
			Thread.sleep(3000);
			selenium.clickElement(iOSClientDriver, iOSClient.HISTORY_SCREEN_DELETE_ALL_HISTORY_CONFIRM);

		} catch (Exception e) {
			message = String.format("*** deleteAllHistory *** FAILED - %s.", e.toString());
			logger.error(message);
			throw new AssertionError(message);
		}
	}
	...
	/**
	 * 
	 * Verify Callee Name before accept call
	 * 
	 * @author HuyD
	 * 
	 */
	public void verifyContactNameCommingCallBeforeAccept(AppiumDriver iOSClientDriver, String Expected)
			throws Exception {
		try {
			Thread.sleep(1000);
	...
	/**
	 * 
	 * Verify Callee Name after accept call
	 * 
	 * @author HuyD
	 * 
	 */
	public void verifyContactNameCommingCallAfterAccept(AppiumDriver iOSClientDriver, String Expected)
			throws Exception {
		try {
			String CalleeName = selenium.getAttribute(iOSClientDriver,
					iOSClient.INCOMING_CALL_CONTACT_NAME_AFTER_ACCEPT_CALL, "value");
			if (CalleeName.equalsIgnoreCase(Expected)) {
				logger.info("PASSED");
			} else {
				logger.error("verifyContactName - Failed");
	...
	/**
	 * 
	 * Make call by input keypad number
	 * 
	 * @author HuyD
	 * 
	 */
	public void makeCallByKeypad(AppiumDriver iOSClientDriver, String number) throws Exception {
		String message = String.format("*** makeCallByKeypad - make a call ***");
		try {
			Thread.sleep(1000);
			selenium.clickElement(iOSClientDriver, iOSClient.ACIOS_WD_CALL_BTN_OPEN_KEYPAD);
			String[] num = number.split("");

			for (int i = 0; i < num.length; i++) {
				selenium.clickElement(iOSClientDriver, iOSClient.DIALPAD_BTN(num[i]));
				logger.info(num[i]);
			}
			selenium.clickElement(iOSClientDriver, iOSClient.ACIOS_WD_POPUP_DIALPAD_BTN_CALL);

		} catch (Exception e) {
			message = String.format("*** makeCallByKeypad *** FAILED - %s.", e.toString());
			logger.error(message);
			throw new AssertionError(message);
		}
	 ...
	 * 
	 */
	public void dropCall(AppiumDriver iOSClientDriver) throws Exception {
		String message = String.format("*** dropCall - Drop a call ***");
		logger.warn(message);
		try {
			if (selenium.isElementExisted(iOSClientDriver, iOSClient.ACIOS_WD_CALL_BTN_HANGUP)) {
				selenium.clickElement(iOSClientDriver, iOSClient.ACIOS_WD_CALL_BTN_HANGUP);
			}
		} catch (Exception e) {
	...
	public void verifyEnterpriseContactSearch(AppiumDriver iOSClientDriver, String name, String email)
			throws Exception {
		try {
			boolean flag = false;
			logger.info("**** verifyEnterprieseContactSearch - Start verify contact search****");
	...
	public void verifyEnterpriseNumberOfResult(AppiumDriver iOSClientDriver, String email, String expected)
			throws Exception {
		try {
			selenium.inputText(iOSClientDriver, iOSClient.CONTACT_SCREEN_SEARCH_TXT, email);
			Thread.sleep(10000);
			...
			if (!s1.contains(expected)) {
				logger.error("verifyEnterpriseNumberOfResult - FAILED - "
						+ "Expected result didn't exist. Expected result is" + expected + "but actual is" + s1);
	...
	public boolean verifyLocalContactSearch(AppiumDriver iOSClientDriver, String name, String email) throws Exception {
		try {
			boolean flag = false;
			logger.info("**** verifyLocalContactSearch - Start verify contact search****");
			logger.info("verifyLocalContactSearch - Search string: " + email);
			selenium.inputText(iOSClientDriver, iOSClient.CONTACT_SCREEN_SEARCH_TXT, email);
			Thread.sleep(10000);
			logger.info("**** verifyLocalContactSearch - Get number of result ****");
			String s1 = selenium.getAttribute(iOSClientDriver, iOSClient.SEARCH_CONTACT_LOCAL_CONTACT_TITLE_BAR,
					"name");
			String stringNumber = s1.replaceAll("[^0-9]", "");
			int num = Integer.parseInt(stringNumber);
			...
			for (int i = 0; i < num; i++) {
				String order = String.valueOf(i + 1);
				String s2 = selenium.getAttribute(iOSClientDriver, iOSClient.local_result_by_order(order), "name");
				logger.info("verifyLocalContactSearch - result " + (i + 1) + ": " + s2);
				if (s2.contains(name)) {
					logger.error("verifyLocalContactSearch - Existed the expected result - PASSED OH YEAH");
					flag = true;
					break;
				}
			}
			...
			logger.error("verifyLocalContactSearch - PASSED OH YEAH");
			
			return flag;
		} catch (Exception e) {
			logger.error("verifyLocalContactSearch - Failed with Exception: " + e + "...\n");
			throw new Exception("verifyLocalContactSearch - Failed - Exception occurs: " + e);
		}
