...
//			capabilities.setCapability("automationName", "uiautomator2");
	...
	public WiniumDriver createWindowsClientDriverOptional(String winiumURL, String App, String os) throws Exception {
		WiniumDriver tempDriver = null;
		try {
			logger.info("createWindowsClientDriver - starting...\n");
			DesktopOptions options = new DesktopOptions();
			
			if (App.equalsIgnoreCase("onex")) {
				if (os.equalsIgnoreCase("64")){
					options.setApplicationPath("C:\\Program Files (x86)\\Avaya\\Avaya one-X Communicator\\onexcui.exe");}
				else {
					options.setApplicationPath("C:\\Program Files\\Avaya\\Avaya one-X Communicator\\onexcui.exe");	
				}
			} else {
				if (os.equalsIgnoreCase("64")){
				options.setApplicationPath("C:\\Program Files (x86)\\Avaya\\Avaya Equinox\\Avaya Equinox.exe");}
				else {
					options.setApplicationPath("C:\\Program Files\\Avaya\\Avaya Equinox\\Avaya Equinox.exe");	
				}
			}
			...
			logger.error("createWindowsClientDriverOptional - Failed with Exception: " + exception + "...\n");
			throw new Exception("createWindowsClientDriverOptional - Failed - Exception occurs: " + exception);
		}
		return tempDriver;
	}
